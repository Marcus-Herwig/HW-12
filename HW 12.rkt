...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |HW 12|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
...
(define block-expression-parser
  (lambda (block-expression)
    (cond
      ((null? block-expression)
       '())
      ((eq? (car block-expression) 'block)
       (cons 'block-exp (block-expression-parser (cdr block-expression))))
      ((list? (car block-expression))  (cons(no-parser (car block-expression)) (block-expression-parser (cdr block-expression))))
       (else "Not a valid block expression"))))
  ...
  (lambda (parsed-block-expression env)
    (cond
      ((null? parsed-block-expression)
       '())
      ((eq? (car parsed-block-expression) 'block-exp) (run-parsed-block-expression (cdr parsed-block-expression) env))
      (else (let* ((body (car parsed-block-expression))
                   (new-env (if(eq? (caar parsed-block-expression) 'remember-exp)
                               (run-parsed-remember-expression body env)
                               env))
                   (output (if(eq? (caar parsed-block-expression) 'remember-exp)
                              'remembered
                              (run-parsed-code body new-env)))
                   (list-of-outputs (cons output (run-parsed-block-expression (cdr parsed-block-expression) new-env))))
            list-of-outputs)))))
...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
       ...
       (run-parsed-block-expression parsed-no-code env))
...
(define sample-block-code '(block (remember a 15) (remember b 17)(spit-out a)(spit-out b) (remember c (do-mathy-stuff + a b)) (spit-out c)))
(define parsed-no-code (no-parser sample-block-code))
;parsed-no-code
